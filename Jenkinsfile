...
"""
    ...
    stages {
        ...
        stage('Maven Build') {
            steps {
                // Groovy의 withEnv 블록을 사용하여 MAVEN_REPO_PATH 변수에 절대 경로를 미리 설정합니다.
                withEnv(["MAVEN_REPO_PATH=${WORKSPACE}/.m2"]) {
                    container('maven') {
                        // Maven 캐시 디렉토리를 생성하고, 미리 설정된 MAVEN_REPO_PATH 환경 변수를 사용합니다.
                        // 이 방식은 셸 확장 오류를 가장 강력하게 방지합니다.
                        sh "mkdir -p \$MAVEN_REPO_PATH && ./mvnw clean package -DskipTests -Dcheckstyle.skip=true -Dmaven.repo.local=\$MAVEN_REPO_PATH"
                    }
                }
            }
        }
